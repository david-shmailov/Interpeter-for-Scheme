...
// - Type definitions  the AST of the language (with type predicates, constructors, getters)
...
const parseForExp = (v: Sexp, body: Sexp[]): Result<ForExp> =>
    ...
    isIdentifier (v) ?
        bind(mapResult(parseL21CExp, body),
         (cexps: CExp[]) => isNumExp(cexps[0]) && isNumExp(cexps[1]) ? makeOk(makeForExp(makeVarDecl(v) ,cexps[0], cexps[1],cexps[2])):
