...
export const makeExtEnv = (vs: string[], cexp: CExp[], env: Env): ExtEnv =>
    ({tag: "ExtEnv", vars: vs, vals: cexp, nextEnv: env});
...
export const applyEnv = (env: Env, v: string): Result<CExp> =>
    ...
    isExtEnv(env) ? applyExtEnv(env, v): makeFailure(" left recenv") ;
  //  applyRecEnv(env, v);
