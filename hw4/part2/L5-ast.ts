...
import {  isToken, isSexpString, parse } from "../shared/parser";
...
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isValuesExp(x)|| isLetValuesExp(x)|| isLetExp(x) || isLetrecExp(x) || isSetExp(x);
...
export interface BindingValue {tag: "BindingValue"; vard: VarDecl[]; val: ValuesExp ; }
export const makeBindingValue = (v: VarDecl[], val: ValuesExp): BindingValue =>
    ({tag: "BindingValue", vard: v, val: val});
export const isBindingValue = (x: any): x is BindingValue => x.tag === "BindingValue";
...
export interface ValuesExp {tag: "ValuesExp"; val: CExp[]; }
export const makeValuesExp = (val: CExp[]): ValuesExp =>
    ({tag: "ValuesExp",val: val});
export const isValuesExp = (x: any): x is ValuesExp => x.tag === "ValuesExp";

export interface LetValuesExp {tag: "LetValuesExp"; bindings: BindingValue[] ;body: CExp[]; }
export const makeLetValuesExp = (vars: BindingValue[] , val:  CExp[]): LetValuesExp =>
    ({tag: "LetValuesExp", bindings: vars ,body: val});
...
export const parseValuesExp = (params: Sexp[]) : Result<ValuesExp> =>
...
export const parseLetValuesExp = (bindings: Sexp , body:  Sexp[] ) : Result<LetValuesExp> =>
    ...
    ! isGoodBindingsValue(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`) :
    safe2((bdgs: BindingValue[], body: CExp[]) => makeOk(makeLetValuesExp(bdgs, body)))
    (parseOneBindValues(bindings), mapResult(parseL5CExp, body));
...
const isGoodBindingsValue = (bindings: Sexp): bindings is [Sexp[], Sexp[]][] =>
    isArray(bindings) && allT(isArrayOfArrayPairs, bindings);

const isArrayOfArrayPairs = (arr: any):arr is any[][] => isArray(arr) && allT(isArray,arr);
...
// const parseBindingsValue = (bindings: [Sexp[], Sexp[]][]): Result<BindingValue[]> =>
//     safe2((vds: VarDecl[], vals: ValuesExp) => makeOk(zipWith(makeBinding, vds, vals.val)))
//         (mapResult(parseVarDecl, map(b => b[0], bindings)), mapResult(parseL5CExp, map(b => b[1], bindings)));

const parseOneBindValues = (bindings: [Sexp[], Sexp[]][]):Result<BindingValue[]> =>
    safe2((vds: VarDecl[][], vals: CExp[]) => makeOk(zipWith(makeBindingValue, vds, vals)))
        (mapResult(parseAllBindingsArray, map(b => b[0], bindings)), mapResult(parseValuesExp, map(b => b[1], bindings)));


const parseAllBindingsArray = (x: Sexp[]): Result<VarDecl[]>=>
    mapResult(parseVarDecl, x);
...
const unparseValuesExp = (les: ValuesExp): Result<string> =>
    bind(mapResult(unparse, les.val), (les: string[]) => makeOk('(values '+join(" ", les) +')'));

const unparseLetValues = (le: LetValuesExp): Result<string> =>
    safe2((bdgs: string, body: string) => makeOk(`(let-values (${bdgs}) ${body})`))
        (unparseBindingsvalue(le.bindings), unparseLExps(le.body));

 const unparseBindingsvalue = (bindings: BindingValue[]): Result<string> =>{
     const a =mapResult(unparseBindingValue, bindings);
     return isOk(a)?makeOk ("("+a.value.join(" ")+")"): a; 

   
 }
 const unparseBindingValue= (b : BindingValue): Result<string> =>{

    const s= mapResult( unparseVarDecl ,b.vard);
        
    const s2= isOk(s)? s.value.join(" "): " error unparse bindingvalues ";
  
    const t= unparseValuesExp(b.val) ;
    const t2= isOk(t)? t.value :" error unparse bindingvalues ";
    
    return makeOk("(("+s2+") "+t2+")" );

}
// const values = parseL5 ("L5 (1)")
// const a = parseL5("(L5 (let-values (((a b c) (values 1 3 5))) (+ a b) ))")
// const s= parseL5("(L5 (let-values (((a b c) (values 1 3 5))((d e f) (values 8 9 10))) (+ a b) ))");
// const b = isOk(values)? unparse(values.value): values;
// isOk(b)? console.log(b.value):console.log(b);